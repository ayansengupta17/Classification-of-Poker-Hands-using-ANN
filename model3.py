...
def initialize_w2(train_data):
	...
	w3 = 2*np.random.random((18,10)) -1
	w4 = 2*np.random.random((10, 10)) -1
	weights = [w1, w2, w3, w4]
	return weights
...
def stochastic_model(train_data, output, step, mf, iterations, lambd, weights):
	...
	for j in range(iterations):
		...
		for k in range(len(train_data)):
			...
			l_4_error = np.array([output[k]]) - l_4
...
def accuracy2(w, train_data, labels):
	...
	for i in range(len(l_4)):
		...
		if list(l_4[i]).index(max(l_4[i])) == list(labels[i]).index(max(labels[i])):
			k +=1
			#print(k)
	acc = k/len(l_4)*100
...
def predict(w, test_data, labels,file):
	...
	predict = []
	for i in range(len(l_4)):
		predict.append(list(l_4[i]).index(max(l_4[i])))

	id_ = [i for i in range(len(l_4))]
